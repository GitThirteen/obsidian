...
export namespace obsidian 
{
    ...
    public:
        ...
        auto compile_shader(const vk::raii::Device& device, std::string_view file_path, std::string_view entry_point, SlangStage stage) -> Result<vk::raii::ShaderModule>
        {
            ...
            {
                return std::unexpected("Failed to create Slang compilation session.");
            }
            ...
            {
                if (any_errors) return std::unexpected(std::format("Slang Compilation Error ({}): {}", file_path, diagnostics));
            ...
            if (!spirv_data || spirv_size == 0) {
                return std::unexpected("Slang returned empty SPIR-V binary.");
            }
            ...
            try {
                ...
                return std::unexpected(std::format("Failed to create Vulkan Shader Module: {}", e.what()));
            }
        }
        ...
        {
            return std::unexpected("Pipeline building not fully implemented yet.");
        }
    ...
    private:
        ...
        {
            return std::unexpected(std::format("Failed to build graphics pipeline: {}", e.what()));
        }
    };
}