...
void SceneManager::switch_to(const std::string& name)
{
	...
	// Lazy Init: Only load resources when we actually enter the scene
	this->m_active_scene->initialize(this->m_root, this->m_window);
...
{
	if (!this->m_active_scene) return;
	
	this->m_active_scene->update(dt);
}
...
void SceneManager::process_input()
{
	...
	if (glfwGetKey(this->m_window.get_window(), GLFW_KEY_C) == GLFW_PRESS)
	{
