...
void ShaderManager::build_pipeline(const std::string& name, const PipelineOptions& options, avk::renderpass renderpass, uint32_t push_constant_size)
{
...
void ShaderManager::build_graphics_pipeline(Pipeline& pipeline, const PipelineOptions& options, avk::renderpass renderpass, uint32_t push_constant_size)
{
	...
	{
		renderpass = this->m_root.default_renderpass();
	}
